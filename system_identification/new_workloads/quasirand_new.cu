...
static int g_num_elements = 1 << 24;
static int jobs = 1;
...
static std::vector<unsigned int> h_out;
static unsigned int* d_out = nullptr;

static std::ofstream responsetime_log;
static std::ofstream summary_log;

static std::ofstream output_csv;
static std::mutex interval_mutex;
static std::vector<float> interval_response_times_ms;
...
double returntempjetson() {
    const std::vector<std::string> candidates_mc = {
        "/sys/class/thermal/thermal_zone1/temp",
    };

    for (const auto& p : candidates_mc) {
        double mc = read_double_from_file(p);
        if (mc > 0.0) return mc / 1000.0;
    }
    return -1.0;
}
...
void monitorpower_and_freq(int sample_period_ms = 200) {
    while (monitor_running.load(std::memory_order_relaxed)) {
        auto now = Clock::now();
        double t_ms =
            std::chrono::duration<double, std::milli>(now - program_start_time).count();

        double p = returnpowerjetson();
        double f = returnfreqjetson();
        double temp = returntempjetson();
        ...
        {
            std::lock_guard<std::mutex> lg(interval_mutex);
            num_completed = static_cast<int>(interval_response_times_ms.size());
            if (!interval_response_times_ms.empty()) {
                float sum = 0.0f;
                for (float x : interval_response_times_ms) sum += x;
                avg_rt = sum / static_cast<float>(interval_response_times_ms.size());
                interval_response_times_ms.clear();
            }
        }
...
__global__ void quasirandKernel(unsigned int* out, int n, unsigned int seed) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) {
        unsigned int x = seed ^ static_cast<unsigned int>(idx);
        #pragma unroll 16
        for (int i = 0; i < 64; ++i) {
            x = xorshift32(x);
        }
...
void init_memory() {
    h_out.resize(g_num_elements, 0);
    CUDA_CHECK(cudaMalloc(&d_out, g_num_elements * sizeof(unsigned int)));
    CUDA_CHECK(cudaMemcpy(d_out, h_out.data(), g_num_elements * sizeof(unsigned int), cudaMemcpyHostToDevice));
...
void task(int id) {
    ...
    for (int i = 0; i < jobs; ++i) {
        ...
        quasirandKernel<<<gridDim, blockDim>>>(d_out, g_num_elements, 1234567u + static_cast<unsigned int>(i));
    ...
    CUDA_CHECK(cudaMemcpy(h_out.data(), d_out, g_num_elements * sizeof(unsigned int), cudaMemcpyDeviceToHost));
    ...
    {
        std::lock_guard<std::mutex> lg(interval_mutex);
        interval_response_times_ms.push_back(response_ms);
    }
...
static void save_results_json(const std::string& path) {
    ...
    out << "{\n";
    out << "  \"platform\": \"jetson\",\n";
    out << "  \"workload\": \"quasirand_releaseguard\",\n";
...
int main(int argc, char* argv[]) {
    ...
    std::string out_json = path + "/summary.json";
    std::string csv_path = path + "/results.csv";
    std::string rt_path = path + "/rt.txt";
    std::string summary_path = path + "/summary.txt";

    output_csv.open(csv_path, std::ios::out);
    output_csv << "window_end_ms,freq_mhz,avg_response_ms,power_w,temp_c,num_completed\n";
    responsetime_log.open(rt_path, std::ios::out);
    summary_log.open(summary_path, std::ios::out);
