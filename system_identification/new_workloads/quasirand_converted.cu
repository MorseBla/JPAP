...
int main(int argc, char* argv[]) {
    ...
    std::string out_json = "logs/quasirand_releaseguard.json";
    if (argc >= 6) out_json = argv[5];

    std::filesystem::create_directories("logs");
    responsetime_log.open("logs/rt_quasirand.txt", std::ios::out);
    summary_log.open("logs/summary_quasirand.txt", std::ios::out);
