    ...
    while (std::getline(file, line)) {
        if (!line.empty() && line[0] == 't') {
            ...
            if (it != mappingTable.end()) {
                ...
                // Assign values to proc1, proc2, proc3, proc4 based on the mapping
                switch (currentLine) {
                    case 1:
                        proc11 = value2;
                        break;
...
int main(int argc, char*argv[])
{
    ...
    //sharedData->newperiods[proc33]=0;
    //sharedData->newperiods[proc44]=0;
    float setpoint1;
    float setpoint2;    
    float setpoint3;  
    float setpoint4; 
    ...
    start:
        std::ifstream file("logs/task_values.txt");
        std::string task1, task2,task3,task4;
        file >> task1 >> task2>>task3>>task4;
        file.close();
        t1 = task1;
        t2 = task2;
        t3 = task3;
        ...
        file.close();
        tasks = 0;
        if (!task1.empty()) tasks++;
        if (!task2.empty()) tasks++;
        if (!task3.empty()) tasks++;
        if (!task4.empty()) tasks++;
        ...
        std::cout << t1 << "\t" << t2 << "\t" << t3 << "\t" << t4 << std::endl;   
         ...
         if(tasks==3)
        {
            signal(mappingTable[task1], signalHandler2);
            signal(mappingTable[task2], signalHandler2);
            signal(mappingTable[task3], signalHandler2);
        ...
        {
            signal(mappingTable[task1], signalHandler3);
            signal(mappingTable[task2], signalHandler3);
            signal(mappingTable[task3], signalHandler3);
            signal(mappingTable[task4], signalHandler3);
