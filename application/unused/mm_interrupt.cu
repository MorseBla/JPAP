...
private:
    ...
    void do_workload_change(dim3& gridDim, dim3& blockDim);
...
void service::do_workload_change(dim3& gridDim, dim3& blockDim)
{
...
void service::runService()
{
    ...
    while (keepRunning) {
        ...
        if (!workload_changed && elapsed_sec >= (termination / 2.0)) {
            std::cout << "Workload change\n";
            if (asyncJob.valid()) {
                prev_rt_ms = asyncJob.get();
            }
            workload_changed = true;
            do_workload_change(gridDim, blockDim);
            first_job = true;
            prev_rt_ms = 0.0;
        }
