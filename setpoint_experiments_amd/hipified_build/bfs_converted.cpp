...
float service::kernellaunch(dim3 gridDim, dim3 blockDim, double carry_ms) {
    ...
    for (int i = 0; i < jobs; ++i) {
        ...
        for (int iter = 0; iter < iterations; ++iter) {
            bool over = false;
            CUDA_CHECK(hipMemcpyAsync(d_over, &over, sizeof(bool), hipMemcpyHostToDevice, stream));
...
void service::runService() {
    ...
    CUDA_CHECK(hipMalloc(&d_nodes, sizeof(Node) * no_of_nodes));
    CUDA_CHECK(hipMalloc(&d_edges, sizeof(int) * no_of_edges));
    CUDA_CHECK(hipMalloc(&d_mask, sizeof(unsigned char) * no_of_nodes));
    CUDA_CHECK(hipMalloc(&d_updating_mask, sizeof(unsigned char) * no_of_nodes));
    CUDA_CHECK(hipMalloc(&d_visited, sizeof(unsigned char) * no_of_nodes));
    CUDA_CHECK(hipMalloc(&d_cost, sizeof(int) * no_of_nodes));
    CUDA_CHECK(hipMalloc(&d_over, sizeof(bool)));

    CUDA_CHECK(hipMemcpy(d_nodes, h_nodes.data(), sizeof(Node) * no_of_nodes, hipMemcpyHostToDevice));
    CUDA_CHECK(hipMemcpy(d_edges, h_edges.data(), sizeof(int) * no_of_edges, hipMemcpyHostToDevice));
    CUDA_CHECK(hipMemcpy(d_mask, h_mask.data(), sizeof(unsigned char) * no_of_nodes, hipMemcpyHostToDevice));
    CUDA_CHECK(hipMemcpy(d_updating_mask, h_updating_mask.data(), sizeof(unsigned char) * no_of_nodes, hipMemcpyHostToDevice));
    CUDA_CHECK(hipMemcpy(d_visited, h_visited.data(), sizeof(unsigned char) * no_of_nodes, hipMemcpyHostToDevice));
    CUDA_CHECK(hipMemcpy(d_cost, h_cost.data(), sizeof(int) * no_of_nodes, hipMemcpyHostToDevice));
    ...
    CUDA_CHECK(hipFree(d_mask));
    CUDA_CHECK(hipFree(d_updating_mask));
    CUDA_CHECK(hipFree(d_visited));
    CUDA_CHECK(hipFree(d_cost));
    CUDA_CHECK(hipFree(d_over));
}
