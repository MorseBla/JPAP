...
static void print_cuda_versions_or_exit()
{
    ...
    if (e1 != hipSuccess || e2 != hipSuccess) {
        std::cerr << "Failed to query CUDA versions. driverGetVersion="
                  << hipGetErrorString(e1) << " runtimeGetVersion="
...
void service::runService()
{
    ...
    CUDA_CHECK(hipHostAlloc((void**)&h_A, bytes, hipHostMallocDefault));
    CUDA_CHECK(hipHostAlloc((void**)&h_B, bytes, hipHostMallocDefault));
    CUDA_CHECK(hipHostAlloc((void**)&h_C, bytes, hipHostMallocDefault));
    ...
    CUDA_CHECK(hipMemcpyAsync(d_A, h_A, bytes, hipMemcpyHostToDevice, stream));
    CUDA_CHECK(hipMemcpyAsync(d_B, h_B, bytes, hipMemcpyHostToDevice, stream));
    CUDA_CHECK(hipMemcpyAsync(d_C, h_C, bytes, hipMemcpyHostToDevice, stream));
