...
const unsigned int num_elements = 1 << 25;
...
void monitorpower_and_freq(int sample_period_ms = 5) {  // 5 ms
...
class HistogramBenchmark {
  public:
    HistogramBenchmark(int repeats_, int jobs_) : repeats(repeats_), jobs(jobs_) {}

    void init() {
        host_input.resize(num_elements);
        host_bins.resize(num_bins, 0);

        std::srand(1);
        for (unsigned int i = 0; i < num_elements; ++i) {
            host_input[i] = std::rand() % num_bins;
        }
        ...
        CUDA_CHECK(cudaMemcpy(d_input, host_input.data(),
                              num_elements * sizeof(unsigned int), cudaMemcpyHostToDevice));

        blockDim = dim3(256);
        gridDim = dim3((num_elements + blockDim.x - 1) / blockDim.x);
    }
    ...
    ResultRow run_one_frequency(int target_freq_mhz) {
        ...
        std::thread mon(monitorpower_and_freq, 5);  // every 5 ms
        ...
        for (int rep = 0; rep < repeats; ++rep) {   // 20 runs
            ...
            for (int j = 0; j < jobs; ++j) {
                ...
                histogram_kernel<<<gridDim, blockDim, 0, stream>>>(
                    d_input, d_bins, num_elements, num_bins);
  ...
  private:
    ...
    std::vector<unsigned int> host_input;
    std::vector<unsigned int> host_bins;
...
int main(int argc, char** argv) {
    std::string out_json = "histogram_freq_power_exec";
    ...
    HistogramBenchmark bench(repeats, jobs);
    bench.init();
